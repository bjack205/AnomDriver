...
% MDP: parameters defining the MDP
    ...
    % MDP.lookup: array of state indexes that need updating
...
% initialize counts
N = ones(MDP.num_states, MDP.num_actions, MDP.num_states); % count for N(s,a,s')
rho = zeros(MDP.num_states, MDP.num_actions); % count for rho(s,a)
epoch = 0;
NOCLT = 0;
iter = 1;
...
while (NOCLT < params.NO_LEARNING_THRESHOLD)
    ...
    for i = 1:params.num_update
        % query simulator 
        state_ind = state_to_index_reduced(state,MDP.lookup);
        ...
        new_state_ind = state_to_index_reduced(new_state,MDP.lookup);
